...
app.get('/sendMessage', function (req, res) {
  ...
  conversation.message({
    ...
    context: context2
      ...
      var msgOut = "<br>" + "<div class=\"cb_att\">" +
				  "<div class=\"cb_att_title\">@Watson</div>" + response.output.text[0] + "</div></div>";

      if(response.output.text.length > 1)
        for(var i = 1; i < response.output.text.length; i++)
          msgOut += "<br>" + "<div class=\"cb_att\">" +
				  "<div class=\"cb_att_title\">@Watson</div>" + response.output.text[i] + "</div></div>";
      res.send(msgOut)
  });
